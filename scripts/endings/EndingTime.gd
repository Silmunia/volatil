...
func UpdateText():
	get_node("Title").set_text("Demitido")
	var chance = randf()
	if (chance >= 0.66):
		get_node("Desc").set_text("Você demorou demais! Seja mais ágil no seu próximo trabalho")
	elif (chance >= 0.33):
		get_node("Desc").set_text("Pouco tempo? Trate vários resíduos ao mesmo tempo na próxima!")
	else:
		get_node("Desc").set_text("Não perca tempo! Tire o resíduo do equipamento logo que possível!")
