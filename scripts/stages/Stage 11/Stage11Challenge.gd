...
func _ready():
	...
	residueCounter = 0
	residueGoal = 15
	get_node("HUD").SetPauseCurrentStage("res://scenes/stages/Stage 11/Stage11Challenge.tscn")
...
func _on_MainCharacter_updateHealth(characterState, health):
	if (characterState == "Health"):
		GameMaster.SetStageEndInfo("11", stageType, residueCounter)
...
func _on_Storage_residueFinished():
	...
	if (residueCounter == residueGoal):
		...
		GameMaster.SetStageEndInfo("11", stageType, timeFinished)
