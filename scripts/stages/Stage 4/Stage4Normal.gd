...
func _ready():
	...
	flowControl = true
	residueCounter = 0
	get_node("HUD").SetPauseCurrentStage("res://scenes/stages/Stage 4/Stage4Normal.tscn")
...
func _on_MainCharacter_updateHealth(characterState, health):
	if (characterState == "Health"):
		GameMaster.SetStageEndInfo("4", stageType, residueCounter)
...
func _on_FlowTimer_timeout():
	...
	elif (stageState == 6 or stageState == 11 or stageState == 17 or stageState == 22 or stageState == 26 or stageState == 31 or stageState == 35 or stageState == 40 or stageState == 44 or stageState == 49 or stageState == 54):
		stageState -= 1
	elif (stageState == 55):
		stageState = 56
...
func StageStateMachine():
	match stageState:
		...
		13:
			get_node("HUD").get_node("CanvasLayer/Messages/Hint/TextureRect/Label").set_text("Agora que você tem o resíduo de MERCÚRIO em mãos, é hora de começar o tratamento")
			TutorialFlowControl(1, 5, "Hint")
		...
		14:
			...
			TutorialCheckAdvance(1, "Balance", 18)
		15:
			get_node("HUD").get_node("CanvasLayer/Messages/Hint/TextureRect/Label").set_text("A primeira etapa do tratamento do resíduo de mercúrio é AJUSTAR O PH")
			TutorialFlowControl(1, 5, "Hint")
			TutorialCheckResidueError(0, null)
			TutorialCheckAdvance(1, "Balance", 18)
		16:
		...
		23:
			TutorialFlowControl(0, 1, null)
		24:
			get_node("HUD").get_node("CanvasLayer/Messages/Hint/TextureRect/Label").set_text("A segunda etapa deste tratamento é colocar o resíduo para REPOUSAR")
			TutorialFlowControl(1, 5, "Hint")
			TutorialCheckResidueError(1, "Balance")
			TutorialCheckAdvance(1, "Rester", 27)
		25:
			TutorialFlowControl(0, 1, null)
			TutorialCheckResidueError(1, "Balance")
			TutorialCheckAdvance(1, "Rester", 27)
		26:
			get_node("HUD").get_node("CanvasLayer/Messages/Hint/TextureRect/Label").set_text("Para colocar um resíduo em repouso, use qualquer REPOUSADOR disponível")
			if (flowControl):
				var objectsGroup = get_tree().get_nodes_in_group("Rester")
				for object in objectsGroup:
					object.get_node("AnimationPlayer").play("BlinkLights")
				get_node("HUD").get_node("CanvasLayer/Messages/Hint/ProfPortrait").set_frame(0)
				get_node("HUD").get_node("CanvasLayer/Messages/Hint/ProfPortrait").play()
				...
				get_node("FlowTimer").start()
				flowControl = false
			TutorialCheckResidueError(1, "Rester")
			TutorialCheckAdvance(1, "Rester", 27)
		27:
			TutorialFlowControl(0, 1, null)
		28:
		...
		29:
			TutorialFlowControl(0, 1, null)
		30:
			get_node("HUD").get_node("CanvasLayer/Messages/Hint/TextureRect/Label").set_text("Como você pode ver pela luz verde, o resíduo já pode ser retirado do repouso")
			if (flowControl):
				get_node("HUD").get_node("CanvasLayer/Messages/Hint/ProfPortrait").set_frame(0)
				get_node("HUD").get_node("CanvasLayer/Messages/Hint/ProfPortrait").play()
				get_node("HUD").get_node("CanvasLayer/Messages").show()
		...
		32:
			TutorialFlowControl(0, 1, null)
		33:
			get_node("HUD").get_node("CanvasLayer/Messages/Hint/TextureRect/Label").set_text("A terceira etapa do tratamento do resíduo de mercúrio é FILTRAR")
			TutorialFlowControl(1, 5, "Hint")
			TutorialCheckResidueError(1, "Rester")
			TutorialCheckAdvance(1, "Filter", 36)
		34:
			TutorialFlowControl(0, 1, null)
			TutorialCheckResidueError(1, "Rester")
			TutorialCheckAdvance(1, "Filter", 36)
		...
		41:
			TutorialFlowControl(0, 1, null)
		42:
			get_node("HUD").get_node("CanvasLayer/Messages/Hint/TextureRect/Label").set_text("A quarta etapa para o resíduo de mercúrio é VERIFICAR a eficiência da reação")
			TutorialFlowControl(1, 5, "Hint")
			TutorialCheckResidueError(1, "Filter")
			TutorialCheckAdvance(1, "Checker", 45)
		43:
			TutorialFlowControl(0, 1, null)
			TutorialCheckResidueError(1, "Filter")
			TutorialCheckAdvance(1, "Checker", 45)
		...
		52:
			...
			TutorialCheckAdvance(2, null, null)
		53:
			get_node("HUD").get_node("CanvasLayer/Messages/Hint/TextureRect/Label").set_text("Leve o resíduo de mercúrio até o equipamento indicado para ARMAZENAR")
			if (flowControl):
