...
func _ready():
	...
	residueCounter = 0
	residueGoal = 10
	get_node("HUD").SetPauseCurrentStage("res://scenes/stages/Stage 5/Stage5Challenge.tscn")
...
func _on_MainCharacter_updateHealth(characterState, health):
	if (characterState == "Health"):
		GameMaster.SetStageEndInfo("5", stageType, residueCounter)
...
func _on_Storage_residueFinished():
	...
	if (residueCounter == residueGoal):
		...
		GameMaster.SetStageEndInfo("5", stageType, timeFinished)
