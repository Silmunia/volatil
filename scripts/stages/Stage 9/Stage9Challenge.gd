...
func _ready():
	...
	residueCounter = 0
	residueGoal = 10
	get_node("HUD").SetPauseCurrentStage("res://scenes/stages/Stage 9/Stage9Challenge.tscn")
...
func _on_MainCharacter_updateHealth(characterState, health):
	if (characterState == "Health"):
		GameMaster.SetStageEndInfo("9", stageType, residueCounter)
...
func _on_Storage_residueFinished():
	...
	if (residueCounter == residueGoal):
		...
		GameMaster.SetStageEndInfo("9", stageType, timeFinished)
